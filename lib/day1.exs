defmodule Day1 do
  ...
  def apply_instr({t, d}, {pos, dir}) do
    ...
    IO.puts(
      "#{t}#{d}: (#{elem(pos, 0)},#{elem(pos, 1)}),#{dir} -> (#{elem(new_pos, 0)},#{elem(new_pos, 1)}),#{new_dir}"
    )
...
result =
  Enum.reduce_while(moves, {{{0, 0}, :N}, %{}}, fn {turn, d}, {{pos, dir}, visited} ->
    new_dir = Day1.turn(dir, turn)

    state =
      Enum.reduce_while(1..d, {pos, visited}, fn _, {pos, visited} ->
